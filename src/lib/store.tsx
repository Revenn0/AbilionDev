...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const bootKeys = sessionQueueKeys(session.state.user?.id)
  const queueKeys = useRef(bootKeys)
  const removedFunnelIds = useRef(loadIdSet(bootKeys.removedFunnels))
  const removedLeadIds = useRef(loadIdSet(bootKeys.removedLeads, LEAD_REMOVED_CAP))
  ...
  const settingsDirty = useRef(loadFlag(bootKeys.pendingSettings))
