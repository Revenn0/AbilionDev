...
import { STAGE_LABEL, TEMP_LABEL } from "@/lib/labels"
...
export function ConversationsPage() {
  ...
  const ste = state.settings.steWelcome
        ...
        {rows.length === 0 ? (
          <section className="surface grid place-items-center px-6 py-16 text-center">
            ...
            <p className="mt-1 max-w-md text-[13px] text-muted-foreground">
              Join no grupo ou /start abre o chat da Sté, com memória individual. O mapa da campanha manda — não um «oi» solto.
            </p>
          </section>
        ) : (
            ...
            {lead && (
              <div className="flex flex-col p-5">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="text-[12.5px] text-muted-foreground">
                      Sté · {lead.campaign} · {timeAgo(lead.updatedAt)}
                    </p>
                  </div>
                ...
                <div className="mt-6 space-y-3">
                  <Bubble who="mapa" text={`${STAGE_LABEL[lead.stage]} · ${lead.origin === "private" ? "/start segue o mapa" : lead.lastMessage}`} />
                  {(state.settings.steLinkedTelegram || state.settings.steLinkedWhatsapp) && (
                    <Bubble who="ste" text={ste} />
                  )}
                  {lead.memory && <Bubble who="memoria" text={lead.memory} />}
                  {lead.printAt && (
                    <Bubble who="ester" text="Print do cadastro. A Ester envia a banca — a Sté não inventa isso." />
                  )}
                </div>
              </div>
            )}
...
function Bubble({ who, text }: { who: "ste" | "mapa" | "memoria" | "ester"; text?: string }) {
  if (!text) return null
  const label = who === "ste" ? "Sté" : who === "ester" ? "Ester" : who === "memoria" ? "Memória" : "Campanha"
  return (
