...
- O GET do CRM aplica tombstones locais (`hydrateFunnels`). Apagar um funil antes do hydrate não o ressuscita; o flush seguinte sobe o tombstone. Settings sujos nesta sessão não são pisados pelo GET.
- O painel não faz POST `/api/crm` até o GET hidratar. Um browser novo sem `localStorage` tem um seed local; se o gravasse antes do GET, o `reconcileFunnels` apagava os quadros mais velhos do Worker. Seed só sobe quando o GET veio vazio.
