...
O script reescreve o `t.me/BOT?start=fb_{vid}` no `pointerdown`, no clique e no clique do meio. A `/l` já sai com o visitor no href. O webhook fecha o evento `telegram` com o mesmo visitor.
...
- `safeAppPath` só aceita rotas exactas do painel (e `/fluxo/funil/:id`). `/configuracoesfoo` não redirecciona para Configurações. `/privacidade` no `next=` do login é aceite. `withSafeNext` leva esse `next` no Esqueceu a senha / reset. Já autenticado em `/login?next=` vai para o destino, não para a home. No `pagehide`, o flush usa `keepalive` se o corpo tiver ≤ 60 kb — acima disso o browser corta o beacon; o pending no `localStorage` sobe no próximo login.
