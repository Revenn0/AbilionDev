...
| `GET/POST /api/runtime` | sessão — GET qualquer conta; POST só dono (token, IA, voz). GET junta o username/grupo das settings do KV e marca `settingsUnread` se o Postgres não confirmar |
...
- Dashboard (número grande de capturas), Telegram (joins / Facebook hoje), Conversas (faixa do funil e chips de filtro) e os contadores dos filtros de Leads mostram "…" enquanto o Worker ainda não hidratou. Conversas, o KPI Conversas do Dashboard, joins/Facebook hoje, o passo “Chat iniciado” do Analytics e cada filtro de Leads hidratam pelo recorte certo — cache só de WhatsApp não finge Telegram vazio; inbox com erro e zero naquele recorte é “…” / “Não li”. Não tratam zero como lista vazia.
