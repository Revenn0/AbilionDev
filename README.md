...
| `GET /api/install` | público: manual do pixel + snippet (`?s=` para um script). Sem `s=` o JSON dos 5 passos continua mesmo se o Postgres das definições estiver unread. `?s=` com id de 8 hex e settings unread sem esse script no KV é 503 — não finge “script inexistente”. Se o script está no KV e o funil dele não está no recorte unread (Postgres em baixo, outro quadro leftover), 503 no funil — não devolve o manual sem nome. MCP `abilion_page_install_manual` e `abilion://install/{id}` usam o mesmo critério |
