...
- O índice do CRM mantém esperas e leads com `telegramChatId`. Só o resto (simulação sem chat) corta em 400. O contacto/chat fica num alias permanente. O GET pagina 400; o painel junta até 2000. O Telegram não cria um lead novo só porque o recorte da lista encheu. Dois `/start` ao mesmo tempo no mesmo chat reivindicam o alias antes do UUID — o segundo reusa o id do primeiro.
...
- `safeAppPath` só aceita rotas exactas do painel (e `/fluxo/funil/:id`). `/configuracoesfoo` não redirecciona para Configurações. `/privacidade` no `next=` do login é aceite. `withSafeNext` leva esse `next` no Esqueceu a senha / reset. Já autenticado em `/login?next=` vai para o destino, não para a home. No `pagehide`, o quadro grava no capture antes do flush; o flush usa `keepalive` se o corpo tiver ≤ 60 kb — acima disso o browser corta o beacon; o pending no `localStorage` sobe no próximo login. O snippet do ads (`#pixel`) é sempre `https://www.abilion.lol/t.js`, nunca o origin local.
