...
| `POST /mcp` ou `/api/mcp` | Bearer ou cookie — JSON-RPC para agentes (60 / min por conta e IP). Settings, scripts, funis e import de leads usam o mesmo merge e a mesma cópia Postgres do painel. `abilion_list_leads` com `q=` usa a mesma busca do GET (KV, depois Postgres se o índice estiver oco). Sem `q=`, ids órfãos da página mista vêm do Postgres; se o backup falhar no meio da página a ferramenta marca `clipped` e não inventa lista vazia |
