...
- O índice do CRM mantém esperas e leads com `telegramChatId`. O resto (simulação / captura sem chat) corta em 2000 — o mesmo teto do hydrate. O contacto/chat fica num alias permanente. O GET pagina 400; o painel junta até 25 páginas e só reconcilia quando a última veio sem cursor — lista cortada não apaga o que já estava no browser. O POST `/api/leads` continua a aceitar 120 de cada vez e devolve `saved` + `ids`; o flush só tira da fila esses ids (sem `ids`, só um `saved` igual ao lote inteiro conta) e manda o resto no pedido seguinte (no `pagehide` fica o primeiro lote). Simular 100 /start só mostra sucesso se o Worker gravou o lote todo. O Telegram não cria um lead novo só porque o recorte da lista encheu. Dois `/start` ao mesmo tempo no mesmo chat reivindicam o alias antes do UUID — o segundo reusa o id do primeiro. O CSV exporta a lista hidratada e avisa se bateu nos 2000.
