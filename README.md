...
- A lista antiga da Abilion (contactos em `app_kv`, workspace de 5 Set e o Telegram de 17 Set) volta para o KV `abilion-auth` com `scripts/restore-archived-leads.mts`. Import sem chat fica origem `import` (rótulo Importado, canal WhatsApp) — **não** “Privado /start”. `pagina` vira popup. A Sté **não** avança o quadro nestes contactos (só nota e temperatura). Nomes passam por `resolveLeadName`: MAIÚSCULAS/minúsculas viram título PT (`de`/`dos` ficam baixos); se o “nome” era só o telefone, a ficha mostra o número formatado (`+55 11 98765-4321`) — **não** inventamos pessoa. O nome na ficha é editável. O `/start` do Telegram reaparece com as falas. O dummy de QA (`Lead QA`) não entra. Sem `SUPABASE_SERVICE_ROLE` o Worker não lê essa tabela sozinho.
