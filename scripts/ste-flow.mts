...
const removedFunnelsThrowEnv = { ...runtimeHoleBase, AUTH: kvThrowsOn(runtimeHoleKv, CRM_REMOVED_FUNNELS) } as Env
const goneBoard = {
  ...emptySalesFunnel("Gone"),
  id: "funil-gone",
  production: { name: "Gone", publishedAt: "2026-01-01T00:00:00.000Z", nodes: [], edges: [] },
}
await saveFunnelsKv(runtimeHoleKv, [throwBoard, goneBoard])
await rememberRemovedFunnels(runtimeHoleKv, ["funil-gone"])
const removedFunnelsDownCrmGet = await handleRequest(
  new Request("http://local.test/api/crm", { headers: { cookie: runtimeHoleCookie } }),
  removedFunnelsThrowEnv,
  backgroundCtx()
)
const removedFunnelsDownCrmGetBody = (await removedFunnelsDownCrmGet.json()) as {
  ok?: boolean
  error?: string
  funnels?: Array<{ id?: string }>
  funnelsUnread?: boolean
}
assert(removedFunnelsDownCrmGet.status === 200 && removedFunnelsDownCrmGetBody.ok, "GET CRM tombstone de funil unread ainda manda o leftover")
assert(removedFunnelsDownCrmGet.status !== 503, "GET CRM tombstone de funil unread não esconde o quadro leftover")
assert(removedFunnelsDownCrmGet.status !== 500, "GET CRM tombstone de funil unread não é Falha interna")
assert(removedFunnelsDownCrmGetBody.funnels?.some((item) => item.id === "funil-throw"), "GET CRM tombstone de funil unread não esconde o funil leftover")
assert(!removedFunnelsDownCrmGetBody.funnels?.some((item) => item.id === "funil-gone"), "GET CRM tombstone de funil unread não ressuscita o gone")
assert(removedFunnelsDownCrmGetBody.funnelsUnread === true, "GET CRM tombstone de funil unread marca funnelsUnread")
const removedFunnelsDownMcp = await handleRequest(
  new Request("http://local.test/mcp", {
    method: "POST",
    headers: { "content-type": "application/json", cookie: runtimeHoleCookie, "x-forwarded-for": "203.0.113.189" },
    body: JSON.stringify({ jsonrpc: "2.0", id: 235, method: "tools/call", params: { name: "abilion_list_funnels", arguments: {} } }),
  }),
  removedFunnelsThrowEnv,
  backgroundCtx()
)
const removedFunnelsDownMcpData = JSON.parse(
  ((await removedFunnelsDownMcp.json()) as { result?: { isError?: boolean; content?: Array<{ text?: string }> } }).result?.content?.[0]?.text || "{}"
) as { error?: string; funnels?: Array<{ id?: string }>; unread?: boolean }
assert(removedFunnelsDownMcp.status === 200, "MCP list_funnels tombstone unread não cai em 500")
...
const settingsHookKv = kvThrowsOn(runtimeHoleKv, CRM_SETTINGS)
const settingsHookEnv = { ...runtimeHoleBase, AUTH: settingsHookKv } as Env
assert((await findLeadInKv(settingsHookKv, "@holelead", 0, ""))?.id === "hole-lead", "settings unread ainda lê o leftover")
assert((await loadFunnelsKv(runtimeHoleKv)).some((item) => item.id === "funil-throw"), "settings unread não pisa o funil leftover")
const holeLeadIdsBeforeSettings = (await listLeads(runtimeHoleKv, 40, "all")).filter((item) => item.contact === "@holelead").map((item) => item.id)
const hookSettingsCtx = backgroundCtx()
const hookSettingsPrevFetch = globalThis.fetch
try {
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    if (String(input).includes("api.telegram.org")) {
      return new Response(JSON.stringify({ ok: true }), { status: 200 })
    }
    return hookSettingsPrevFetch(input, init)
  }) as typeof fetch
  const hookSettings = await handleRequest(
    new Request("http://local.test/api/telegram", {
      method: "POST",
      headers: { "content-type": "application/json", "x-telegram-bot-api-secret-token": "hook-kv" },
      body: JSON.stringify({
        ...
        message: {
          chat: { id: 88008 },
          text: "oi settings unread",
          from: { id: 88008, username: "holelead", first_name: "Hole" },
        },
      }),
    }),
    settingsHookEnv,
  ...
  const hookSettingsBody = (await hookSettings.json()) as { ok?: boolean }
  assert(hookSettings.status === 200, "webhook settings unread ainda acks o Telegram")
  assert(hookSettings.status !== 500, "webhook settings unread não é Falha interna")
  assert(hookSettingsBody.ok === true, "webhook settings unread não mente falha no ack")
  await hookSettingsCtx.flush()
} finally {
  globalThis.fetch = hookSettingsPrevFetch
}
const holeLeadIdsAfterSettings = (await listLeads(runtimeHoleKv, 40, "all")).filter((item) => item.contact === "@holelead").map((item) => item.id)
assert(holeLeadIdsAfterSettings.join() === holeLeadIdsBeforeSettings.join(), "webhook settings unread não mint o segundo UUID")
assert((await loadLead(runtimeHoleKv, "hole-lead"))?.id === "hole-lead", "webhook settings unread não apaga o leftover")
assert((await loadLead(runtimeHoleKv, "hole-lead"))?.contact === "@holelead", "webhook settings unread não troca o contacto leftover")
assert((await loadFunnelsKv(runtimeHoleKv)).some((item) => item.id === "funil-throw"), "webhook settings unread não pisa o funil leftover")
const holeAfterSettings = await loadLead(runtimeHoleKv, "hole-lead")
assert(
  (holeAfterSettings?.messages ?? []).some((item) => item.role === "lead" && item.text === "oi settings unread") ||
